...
<!-- authority 1 start -->
   <?php foreach ($exp['time_table'] as $key => $value1) {
if($value1 == 1 && $value != 2){ ?>
  <div class="col-lg-12 class_sec ">
<?php 
  $attributes = array('class' => 'frm');
...
<script type="text/javascript">
  document.getElementById("class_start_time1").disabled = true;
  document.getElementById("class_end_time1").disabled = true;
  <?php for($i1=0;$i1<=5;$i1++)  {  ?>
  document.getElementById("selct_tab<?php echo $i1;  ?>").disabled = true;
  <?php } ?>
</script>
<?php }  ?>
</tr>
...
success : function(data){
 ...
 <?php
foreach ($exp['time_table'] as $key => $value) {
 if($value == '') { ?> 
<script>
