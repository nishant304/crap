...
<tbody>
	<tr class="active">
	    ...
	    <tr>
	      ...
	      <td><input disabled="disabled" style="width: 90px;" id="myInputId4<?php echo  $ebook['notes_id'];?>" name="notes_batch" value="<?php echo $notes_batch; ?>"></td>
	      ...
	      <td id="mylink<?php echo $i; ?>"><a href="<?php echo base_url(); ?>index.php/teacher_controller/ebook_delete/<?php echo $notes_id; ?> " >Delete</a></td> 
			...
			<?php
			foreach ($exp['notes'] as $key => $value1) { }
...
<?php
foreach ($exp['notes'] as $key => $value) {
 if($value == '') { ?> 
<script>
...
<?php 
foreach ($data_stf as $key => $value3) {
}
if($value3 == ''){  ?>
<script>
// condition if authority not set start
document.getElementById("myDIV").style.display = "none";
</script>
<h2 id="err"><?php echo "you have not authority !";?></h2>
<?php } ?>
