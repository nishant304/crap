...
class admission extends CI_Controller
{
  ...
  public function pagination() {
       ...
       $data["results"] = $this->login_model->fetch_departments($config["per_page"], $page);
...
public function daily_basis()
  ...
  $config["total_rows"] = $this->login_model->record_count('test_marks');
  $config["per_page"] = 10;
  $config["uri_segment"] = 3;
  $this->pagination->initialize($config);
  $page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
  $data["results"] = $this->login_model->fetch_departments($config["per_page"],$page,'test_marks');
  $data["links"] = $this->pagination->create_links();
...
// event module start
 ...
 public function participate_stu()
     {
         $event_id = $this->input->post('event_id_ajax');
         $clss = $this->input->post('clss');
         $section = $this->input->post('section');
        // fetch std_id based on event id
        $std_id['std_id_ajax'] = $this->login_model->participate_student($event_id);
    
        // fetch student name based on std id
        $std_all_ajax['std_details'] = $this->login_model->std_id_ajax($std_id,$clss,$section);
        print_r($std_all_ajax);

        ?>
          <select class="form-control">
          <option value="None">select</option> 
          <?php
          foreach ($std_all_ajax['std_details'] as $key) {
          ?>
          <option value="<?php echo $key['std_id'];?>">
          <?php echo $key['std_fname']." ".$key['std_lname'];?></option>
          <?php } ?>
          </select> 
          <?php 
      }
...
for($i=0;$i<count($num);$i++)
{
  ...
  $config["total_rows"] = $this->login_model->record_count($num[$i],$arr_name['a'][$i]);
  $config["per_page"] = 10;
  $config["uri_segment"] = 3;
  $this->pagination->initialize($config);
  $page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
  $data["results".$i] = $this->login_model->fetch_departments($config["per_page"],$page,$num[$i],$arr_name['a'][$i]);
...
public function active_inactive()
{
  ...
  echo $idd = $this->input->post('idd');
  $submit = $this->input->post('submit');
 if($submit == 'inactive'.$idd)
  {
    $status = '1';
  }
  else
  ...
  $this->login_model->active_inactive_status('admission','std_id',$idd,'status',$status);
  redirect('admission/user_listed');
}
...
public function photo_gallery()
{
        ...
        $this->load->view('admin/admin_header');
        $this->load->view('admin/admin_sidebar');
        // $this->load->view('admin/gallery',$a);
        $this->load->view('admin/admin_footer');
...
public function QuizView(){
          ...
          $this->load->view('admin/Quiz/Quiz.php');
...
public function QuizeInsert(){


    $data = array(
      'question' => $this->input->post('question'),
      'optone' => $this->input->post('optone'),
      'opttwo' => $this->input->post('opttwo'),
      'optthree' => $this->input->post('optthree'),
      'optfour' => $this->input->post('optfour'),
      'answer' => $this->input->post('answer'),
      'category' => $this->input->post('Category'),
      'level' => $this->input->post('level'),
  );

$this->login_model->insert_data('quiz_question',$data);

 
redirect('admission/QuizView');
...
public function QuizQuestion(){
          $this->load->view('admin/admin_header.php');
          $this->load->view('admin/admin_sidebar.php');
          $this->load->view('admin/Quiz/question.php');
          $this->load->view('admin/admin_footer.php');
}
public function QusetionView(){
  $data = $this->login_model->QuizFetch('quiz_question');
   echo json_encode($data);
...
public function CheckAns(){
  ...
  for ($i=1; $i <= 2; $i++) { 
    // $name = array(
      $name[] = $this->input->post('id'.$i);
      $n[] = $this->input->post('n'.$i);
    // );
  }
  // print_r($this->input->post('n'.$i));
  print_r($name);
  print_r($n);
  // print_r($this->input->post('qust'.$i));
}

public function QusetionViewans(){
      $result = array(
      'participent_id' => $this->input->post('participent_id'),
      'table_type' => $this->input->post('table_type'),
      'category' => $this->input->post('category'),
      'level' => $this->input->post('level'),
      'ans_right' => $this->input->post('ans_right'),
      'ans_wrong' => $this->input->post('ans_wrong'),
      'score' => $this->input->post('score'),
      'out_of' => $this->input->post('out_of'),
      'last_play_date' => $this->input->post('last_play_date')
    );
      print_r($result);
  // echo "<pre>";
  //     $ids = array(
  ...
  //       'b' => 2
  //     );
      $data = $this->login_model->insert_data('quiz_score',$result);
      // print_r($data);
      echo json_encode($data);
}
